                ...
                <div class="flex-1 min-h-0 overflow-y-auto pr-2">
                    ...
                    >
                        ...
                        <label :for="method.id" class="flex-1 cursor-pointer">
                            ...
                            <div class="text-sm text-muted-foreground">{{ method.norek }}</div>
                        </label>
                    </div>
...
const fetchPaymentMethods = async () => {
    if (!props.room || !props.room.runner_id) return
    ...
    try {
        const { data, error } = await supabase
            ...
            .eq('user_id', props.room.runner_id)
...
const handleMethodSelect = (method) => {
    ...
    if (method.norek) {
        navigator.clipboard.writeText(method.norek)
            .then(() => toast('Copied!'))
